...
void load_theta_rho(void *_args)
{
    ...
    int previous_theta_steps = 0;
    int previous_rho_steps = 0;
    bool previous_ramped_down = true;
    ...
    while (getline(&line, &len, fp) != -1)
    {
        ...
        if (previous_theta_steps != 0 || previous_theta_steps != 0)
        {

            bool ramp_down = (abs(theta_steps) + abs(previous_theta_steps)) > theta_steps ||
                             (abs(rho_steps) + abs(previous_rho_steps)) > rho_steps;
            if (steps_with_speed(
                    previous_theta_steps,
                    previous_rho_steps,
                    1,
                    &is_at_limit,
                    previous_ramped_down ? step_ramp : 0,
                    ramp_down ? step_ramp : 0) != 0)
            {
                printf("%s\n", line);
                printf("lin_pos=%d \t rot_pos=%d\n", lin_pos, rot_pos);
                break;
            }
            previous_theta_steps = theta_steps;
