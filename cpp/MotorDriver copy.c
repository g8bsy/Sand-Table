...
Movement *create_movement(int lin_steps, int rot_steps, int ramp_up, int ramp_down)
{

  Movement *movement = malloc(sizeof(Movement));

  movement->lin_steps = lin_steps;
  movement->rot_steps = rot_steps;
  movement->ramp_up = ramp_up;
  movement->ramp_down = ramp_down;
...
void print_vec_movement(vec_movement *moves)
{
  for (int i = 0; i < (int)vector_size(moves); i++)
  {
    Movement this_move = ((Movement *)moves)[i];
    printf("lin_steps=%d rot_steps=%d ramp_up=%d ramp_down=%d\n",
           this_move.lin_steps,
           this_move.rot_steps,
           this_move.ramp_up,
           this_move.ramp_down);
...
vec_movement *load_theta_rho(char *fname)
{

  vec_movement *moves = vector_create();
  ...
  // TODO - delete
  steps_per_linear = 5591;
  steps_per_revolution = 32000;
  ...
  while (getline(&line, &len, fp) != -1)
  {
    ...
    int theta_steps = theta_coor - last_theta_coor;
    int rho_steps = rho_coor - last_rho_coor;

    // printf("theta_steps=%d \t rho_steps=%d last_theta_coor=%d \t last_rho_coor=%d \t theta=%f \t rho=%f \t theta_coor=%d \t rho_coor=%d \t \n",
    //   theta_steps, rho_steps, last_theta_coor, last_rho_coor, theta, rho, theta_coor, rho_coor
    // );

    Movement *this_move = create_movement(last_theta_coor - theta_coor, rho_steps, 10, 10);
...
int steps_with_speed(int rot_steps, int lin_steps, int delay, bool checkLimit, int ramp_up, int ramp_down)
{
...
void *drive_motors(void *args)
{
  ...
  for (int i = 0; i < (int)vector_size(moves); i++)
  {
    ...
    steps_with_speed(this_move.rot.steps, this_move.lin.steps, 10, true, 10, 10);
  }
...
static PyObject *py_load_theta_rho(PyObject *self, PyObject *args)
{
...
static PyObject *py_steps(PyObject *self, PyObject *args)
{
  ...
  bcm2835_gpio_write(rot_en_pin, LOW);
  bcm2835_gpio_write(lin_en_pin, LOW);
  steps_with_speed(rot_steps, lin_steps, delay, true, 10, 10);
  bcm2835_gpio_write(rot_en_pin, HIGH);
  bcm2835_gpio_write(lin_en_pin, HIGH);
...
static PyObject *py_calibrate(PyObject *self, PyObject *args)
{
  ...
  steps_with_speed(0, 50000, 1, true, 10, 10);

  printf("lin_pos %d\n", lin_pos);

  printf("Move back away from switch\n");
  steps_with_speed(0, -100, 10, false, 10, 10);

  printf("lin_pos %d\n", lin_pos);

  printf("Creep up to switch\n");
  steps_with_speed(0, 200, 100, true, 10, 10);

  printf("lin_pos %d\n", lin_pos);

  printf("Move back away from switch\n");
  steps_with_speed(0, -100, 100, false, 10, 10);
  ...
  printf("Move back away from switch\n");
  steps_with_speed(0, 100, 1, false, 10, 10);
...
static PyMethodDef DrivingMethods[] = {
    ...
    {"load_theta_rho", py_load_theta_rho, METH_VARARGS, "Function to move"},
